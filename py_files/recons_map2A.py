...
Nside = 2048
Lmax = 100
# select approach 1 or approach 2
appr = 3
fname = 'L100_re_const'+str(appr)+'.mat'
...
print(' Npix = ', Npix)
print(' RMS error(hata) = ',np.linalg.norm(err_field)*2.0*np.sqrt(pi)/np.sqrt(Npix))
print(' RMS error(0) = ',np.linalg.norm(org_field)*2.0*np.sqrt(pi)/np.sqrt(Npix))
print(' relative RMS error = ',np.linalg.norm(err_field)/np.linalg.norm(org_field))
...
print('RMS Error where mask=1, err0(hata) = %2.3e' % err1_hata)
print('RMS Error where mask=1, err0(0) = %2.3e' % err1_a0)
print('Relative err where mas>=0 =  %2.3e'  % rel1)
...
err_ti = 'Original field with no noise'
err_sv = 'Original_field_no_noise_QR.png'
cm = cmbcmap()
hp.mollview(org_field, title = err_ti, cmap=cm, min=-70, max=70, xsize=1200, nest=False)
plt.title(err_ti)
plt.savefig(err_sv,format='png',dpi=600) 
